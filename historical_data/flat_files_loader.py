...
# Centralized cache directories (this module lives in historical_data/options_data)
HISTORICAL_DATA_BASE = os.path.dirname(os.path.abspath(__file__))
...
def download_options_trades(date_str, force_refresh=False):
    """
    Download options tick-level trades CSV for a specific date from Massive Flat Files.
    Returns pandas DataFrame or None. Cached as options-trades-YYYY-MM-DD.csv.
    If force_refresh=True, skip cache and re-download from S3 (overwrites cache).
    """
    s3 = get_s3_client()
    if not s3:
        return None
    cache_file = os.path.join(FLAT_FILES_CACHE_DIR, f"options-trades-{date_str}.csv")
    ...
    year = date_obj.strftime('%Y')
    month = date_obj.strftime('%m')
    s3_key = f"us_options_opra/trades_v1/{year}/{month}/{date_str}.csv.gz"
    try:
        response = s3.get_object(Bucket=POLYGON_S3_BUCKET, Key=s3_key)
        content = response['Body'].read()
        try:
...
def download_options_quotes(date_str, force_refresh=False):
    """
    Download options top-of-book quotes CSV for a specific date from Massive Flat Files.
    Returns pandas DataFrame or None. Cached as options-quotes-YYYY-MM-DD.csv.
    If force_refresh=True, skip cache and re-download from S3 (overwrites cache).
    """
    s3 = get_s3_client()
    if not s3:
        return None
    cache_file = os.path.join(FLAT_FILES_CACHE_DIR, f"options-quotes-{date_str}.csv")
    if not force_refresh and os.path.exists(cache_file):
        try:
            return pd.read_csv(cache_file)
        except Exception:
